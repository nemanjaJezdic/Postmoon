    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        try{
                if(!empty($_POST["id"])){
                    ...
                    if($update_result){
                        $rememberlikes=$conn->prepare("DELETE FROM user_posts_likes
                                                       WHERE id=:id");
                        $rememberlikes->bindParam(":id",$iduserlikeresult->id);
                        ...
                        $response["message"]="Successfuly unliked";
                        echo json_encode($response);
                        http_response_code(200);
                    }else{
                        $response["message"]="Could not save unlike";
                        echo json_encode($response);
                        http_response_code(500);
                    }
