...
if (!empty($_GET["search"])) {
  ...
  $resul = $conn->prepare("SELECT p.*,u.id as user_id,u.name as user_name,u.surname as user_surname,c.name as category_name,upl.id as user_post_like_id,COUNT(com.id) as commentnumber
    FROM posts as p 
            ...
            WHERE p.heading LIKE :search
            GROUP BY p.id
            ORDER BY p.created DESC
            LIMIT $this_page,$posts_per_page");

  $resul->bindParam(":userid", $_SESSION["user"]->id);
  $resul->bindParam(":search",$search);
  $resul->execute();
} else {
  $number_of_posts = $conn->query("SELECT COUNT(*) FROM posts")->fetchColumn();
    ...
    <div class="col-9 col-lg-7 text-center border-right">
      ...
      <ul class="pagination pagination-lg justify-content-center mt-3">
        ...
        for ($page = 1; $page <= $number_of_pages; $page++) {
          if(!empty($_GET["search"])){
            echo '<li class="page-item"><a class="page-link" href="posts.php?page=' . $page . '&search='.$_GET["search"].'">' . $page . '</a></li>';
          }else{
